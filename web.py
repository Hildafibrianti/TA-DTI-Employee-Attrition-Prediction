...
PerformanceRating_dict = {'Low':0, 'Good':1, 'Excellent':2, 'Outstanding':3}
...
classifier = joblib.load("random_forest_final.pkl")
...
if st.sidebar.checkbox("Data Visualization", True, key='2'):
    st.sidebar.write("Model Summary")
    st.sidebar.write("Random Forest with parameter:")
    parameter = classifier.get_params()
    for x, y in parameter.items():
        #st.sidebar.write("{} : {}".format(x,y))
        st.sidebar.markdown("<h5 style='text-align: left;'>{} : {} </h5>".format(x,y), unsafe_allow_html=True)
    #Parameter explanation
    st.sidebar.text("")#extra line
    st.sidebar.text("")#extra line
    st.sidebar.write("Parameter dengan koefisien terbesar:")
    for i in range(10):
        #st.sidebar.write("Parameter ke-{}: {}".format(i+1,names[i]))
        st.sidebar.markdown("<p style='text-align: left;font-size:12px;'>Parameter ke-{}: {}</p>".format(i+1,names[i]), unsafe_allow_html=True)
    #Data Visualisasi
    ...
    DailyRate = st.sidebar.number_input("Daily Rate:")
    Department = st.sidebar.selectbox('Department',('Sales', 'Research & Development', 'Human Resources'))
    DistanceFromHome = st.sidebar.number_input("Distance From Home:")
    Education = st.sidebar.selectbox('Education',('Bellow College', 'College', 'Bachelor', 'Master', 'Doctor'))
    ...
    HourlyRate = st.sidebar.number_input("Hourly Rate:")
    JobInvolvement = st.sidebar.selectbox('Job Involvement',('Low', 'Medium', 'High', 'Very High'))
    JobLevel = st.sidebar.number_input("Job Level:")
    JobRole = st.sidebar.selectbox('Job Role',('Sales Executive',
                                        'Research Scientist', 
    ...
    MonthlyIncome = st.sidebar.number_input("Monthly Income:")
    MonthlyRate = st.sidebar.number_input("Monthly Rate:")
    NumCompaniesWorked = st.sidebar.number_input("Number Companies Worked:")
    OverTime = st.sidebar.selectbox('OverTime',('Yes', 'No'))
    PercentSalaryHike = st.sidebar.number_input("Percent Salary Hike:")
    PerformanceRating = st.sidebar.selectbox('Performance Rating',('Low', 'Good', 'Excellent', 'Outstanding'))
    RelationshipSatisfaction = st.sidebar.selectbox('Relationship Satisfaction',('Low', 'Medium', 'High', 'Very High'))
    StockOptionLevel = st.sidebar.number_input("Stock Option Level:")
    TotalWorkingYears = st.sidebar.number_input("Total Working Years:")
    TrainingTimesLastYear = st.sidebar.number_input("Training Times Last Year:")
    WorkLifeBalance = st.sidebar.selectbox('Work Life Balance',('Bad', 'Good', 'Better', 'Best'))
    YearsAtCompany = st.sidebar.number_input("Years At Company:")
    YearsInCurrentRole = st.sidebar.number_input("Years In Current Role:")
    ...
    if submit:
        ...
        pr_lr = PerformanceRating_dict[PerformanceRating]
        ...
        if prediction == 1:
            ...
            indices = np.argsort(important)[::-1] # Sort feature importances in descending order
            names = [graph_df.columns[i] for i in indices] # Rearrange feature names so they match the sorted feature importances
            ...
            for i in df_pred.columns:
                if i == "Overtime ":
                    ...
                    if df_pred[i][0] == 1 or df_pred[i][0] == 2:
                        st.write("Memberikan reward yang menarik")
                elif i == "DistanceFromHome":
                    if df_pred[i][0] < info["{}_mean".format(i)][0]:
                        st.write("Memberikan biaya transport tambahan")
                elif i == "HourlyRate":
                    if df_pred[i][0] < info["{}_mean".format(i)][0]:
                        st.write("Meningkatkan hourly rate karyawan")
                elif i == "DailyRate":
                    if df_pred[i][0] < info["{}_mean".format(i)][0]:
                        st.write("Meningkatkan daily rate karyawan")                                 
        else:
